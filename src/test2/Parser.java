...
exprStmt       → expression ";" ;
forStmt        → "fora" "(" ( varDecl | exprStmt | ";" )
                          expression? ";"
                           expression? ")" statement ;
ifStmt         → "ifa" "(" expression ")" statement
                 ( "elsee" statement )? ;
...
//> Control Flow match-for
  if (match(FORA)) return forStatement();
//< Control Flow match-for
//> Control Flow match-if
  if (match(IFA)) return ifStatement();
//< Control Flow match-if
...
//> Control Flow match-while
  if (match(WHIL)) return whileStatement();
//< Control Flow match-while
//> parse-block
...
private Stmt doWhileStatement() {
    try {
        loopLevel++;
        Stmt body = statement();
        consume(WHIL, "Expect 'while' in a do-while loop.");
